...
class SummaryGenerator:
    ...
    def generate(self):
        ...
        # Format as table
        headers = ['Tool', 'Provider', 'Scans', 'Total', 'Critical', 'High', 'Medium', 'Low']
        print("\n" + "="*80)
        ...
        print(tabulate(top_resources, headers=['Resource Type', 'Region', 'Issues'], tablefmt='grid'))
        ...
        summary = {
            ...
            'statistics': [dict(zip(headers, row)) for row in results],
            'top_resources': [dict(zip(['resource_type', 'region', 'count'], row)) for row in top_resources]
        }
